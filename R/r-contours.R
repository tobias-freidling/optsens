...
df_plot_val <- function(a_seq, b_mat, eval_mat) {
  len_x <- length(a_seq)
  len_y <- dim(b_mat)[2]
  ...
  for (j in names(comparison_ind)) {
    ...
    for (m in comparison_ind[[j]]) {
      if (comparison == "comp") {
        ...
        add_cp_y <- sqrt(m) * r(y, xj, cbind(xxjc, d))
      }
...
iv_lines_list <- function(sa, a_seq, b_mat) {
...
r_contours_data <- function(sa, comparison_ind,
  ...
  grid_list <- feasible_grid(y, d, xt, xp, z, bounds, grid_specs, TRUE, eps)
  list2env(grid_list, environment())
  eval_param <- compute_eval_param(y, d, xt, xp, z)
  eval_mat <- eval_on_grid(a_seq, b_mat, eval_param)
  
  
  df_plot <- df_plot_val(a_seq, b_mat, eval_mat)
  df_cp <- df_comparison_points(sa, comparison_ind, comparison)
  iv_lines_list <- if(iv_lines) iv_lines_list(sa, a_seq, b_mat) else NULL
  
  list(df_plot = df_plot, df_cp = df_cp, iv_lines_list = iv_lines_list)
}



#
r_contours_plot <- function(plot_list, val_interest, bw, n_breaks) {
